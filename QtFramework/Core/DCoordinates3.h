#ifndef DCOORDINATE3_H
#define DCOORDINATE3_H
...
namespace cagd
{
    ...
    public:
        ...
        // (left value) ezt fogja hasznalni ha mosoditod az erteket
        ...
        GLboolean operator ==(const DCoordinate3& rhs) const;
    };
    ...
    // helyigeny es gyorsitas miatt nem mentunk el ideiglenes valtozokat 
    // Figyelj hogy ne legyen nagy a hely es gyorsasag igeny
    // add
    ...
    // miert nem void? ezert = ((a+=1) -= 10) nem mukodne... 
    // += -= es tovabb referencia kell legyen visszateritesre
    // add to *this
    ...
    // inline const, a const azert van hogy ne legyen left value ne vegyen fel ertekeket (ha ossze van adva pl a + b = c)
    // subtract
    inline const DCoordinate3 DCoordinate3::operator -(const DCoordinate3& rhs) const
    {
        return DCoordinate3(_data[0] - rhs._data[0], _data[1] - rhs._data[1], _data[2] - rhs._data[2]);
    ...
    // kell ide temporalis valtozokat
    // a "z" valtozot nyugodtan felulirhatod
    // cross product, result is stored by *this
    inline DCoordinate3& DCoordinate3::operator ^=(const DCoordinate3& rhs)
    {
    ...
    inline GLboolean DCoordinate3::operator ==(const DCoordinate3& rhs) const
    {
